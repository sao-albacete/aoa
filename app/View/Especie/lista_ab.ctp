	...
	<fieldset>
		...
		<table id="tbListaAvesAb" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<th title='<?= __("Estatus Provincial Albacete"); ?>'><a href="#modalEstatusProvincialAb"
																		 data-toggle="modal"><?= __("EP"); ?></a></th>
				<th title='<?= __("Distribución Provincial Albacete"); ?>'><a href="#modalDistribucionProvincialAb"
																			  data-toggle="modal"><?= __("DP"); ?></a>
				</th>
				<th title='<?= __("Estatus Nacional"); ?>'><a href="#modalEstatusNacional"
															  data-toggle="modal"><?= __("EN"); ?></a></th>
				<th title='<?= __("Nivel Protección Castilla-La Mancha"); ?>'><a href="#modalProteccionClm"
																				 data-toggle="modal"><?= __("CR"); ?></a>
				</th>
				<th title='<?= __("Estado de Conservación en España (Libro Rojo)"); ?>'><a href="#modalProteccionLr"
																						   data-toggle="modal"><?= __("LR"); ?></a>
				</th>
			</tr>
			</thead>
			<tbody>
			<?php foreach ($especies_ab as $especie_ab) : ?>
				<?php if (empty($especie_ab['Especie']['subespecie'])) { ?>
					<tr>
						...
						<td style='text-align: center;'><?php if ($especie_ab['Fotos'] > 0) {
								echo('<img src="/img/icons/camera.png" alt="Tiene fotos" title="Tiene fotos"/>');
							} ?></td>
						<td style='text-align: center;'><a href='/especie/view/id:<?= $especie_ab['Especie']['id'] ?>'
														   title='<?= __("Ver ficha de la especie") ?>'><img
									src='/img/icons/search.png' title='Ver ficha de la especie' alt='Ver detalle'/></a>
						</td>
						<td title='<?= $especie_ab['Especie']['nombreIngles'] ?>'><?= $especie_ab['Especie']['nombreComun'] ?></td>
						<td><i><?= $especie_ab['Familia']['nombre'] ?></i></td>
						<td><i><?= $especie_ab['Especie']['genero'] . ' ' . $especie_ab['Especie']['especie'] ?></i>
						</td>
						<td><i><?= $especie_ab['Especie']['subespecie'] ?></i></td>
						<td style='text-align: center; background-color: #B3DCB3;'
							title='<?= $especie_ab['EstatusCuantitativoAb']['nombre'] ?>'><?= $especie_ab['EstatusCuantitativoAb']['codigo'] ?></td>
